...
\title{\textbf{Smart Forest Sentinel: Edge-AI Acoustic Poaching Detection and Environmental Monitoring System}}
\author{
    \textbf{Team Members:} \\
...
\begin{abstract}
This report presents the design, development, and implementation of \textbf{Smart Forest Sentinel (EnviEL)}, an automated, low-power, and cost-effective Edge-AI acoustic monitoring system to combat illegal wildlife poaching (e.g., gunshots, chainsaw logging, vehicle intrusions) in remote forest environments. Developed under the IoT design methodologies detailed in \textit{Internet of Things with Raspberry Pi and Arduino} by Rajesh Singh and Anita Gehlot, the system structures edge sampling, gateway machine learning, and cloud synchronization. Using ESP32 and Arduino Uno nodes, the system achieves local acoustic preprocessing and sends real-time alerts. A local Python edge gateway serial bridge extracts 26-dimensional Mel-Frequency Cepstral Coefficients (MFCC) and classifies audio streams using a Scikit-Learn Random Forest model. Alerts are logged into an atomic, file-based SQLite database configured in WAL mode, synced to a Next.js 15 dashboard, and accessible via a hands-free, voice-controlled Gemini 2.0 Flash AI assistant. Controlled tests demonstrate an overall threat classification accuracy of approximately 86\%, with gunshot detection accuracy exceeding 90\% and an average end-to-end latency of 1.5 seconds.
\end{abstract}

\newpage

\section{Project Objectives}
The primary objective of \textbf{Smart Forest Sentinel} is to establish a scalable, resilient acoustic monitoring network that detects poaching activities in real-time, providing forest rangers with immediate location and severity data.
...
\begin{enumerate}
    ...
    \item \textbf{Scalable Deployment:} Provide a low-cost, modular framework adaptable to remote, resource-constrained forest reserves such as Bandipur National Park.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=2cm]
        \node (sensor) [io] {Acoustic Nodes (ESP32 / Uno)};
        \node (bridge) [process, right=of sensor, xshift=1cm] {Python ML Bridge (MFCC + RF Model)};
        \node (db) [database, below=of bridge, yshift=-0.5cm] {SQLite Database};
        \node (backend) [process, below=of sensor, yshift=-0.5cm] {Next.js API Routes};
        \node (gemini) [startstop, left=of backend, xshift=-0.5cm] {Gemini 2.0 Flash API};
        \node (frontend) [process, below=of backend, yshift=-0.5cm, xshift=3cm] {Ranger Dashboard UI};

        \draw [arrow] (sensor) -- node[above] {Raw Audio} (bridge);
        \draw [arrow] (bridge) -- node[right] {Log Alert} (db);
        \draw [arrow] (sensor) -- node[left] {HTTP Post} (backend);
        \draw [biarrow] (backend) -- (db);
        \draw [biarrow] (backend) -- node[above] {Prompt/JSON} (gemini);
        \draw [biarrow] (frontend) -- node[left] {REST/JSON} (backend);
    \end{tikzpicture}
    \caption{SentinelSound Information Dataflow and Component Diagram}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{lp{5.5cm}p{7cm}}
        \toprule
        \textbf{Stage} & \textbf{Textbook Step} & \textbf{Smart Forest Sentinel Implementation Status} \\
        \midrule
        ...
        9 & Launch of Product & \textbf{Planned:} Field deployment in Bandipur National Forest. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Field Calibration:} Calibrating classification parameters under the canopy of Bandipur National Park to account for humidity, dense vegetation, and animal vocalization.
\end{enumerate}
